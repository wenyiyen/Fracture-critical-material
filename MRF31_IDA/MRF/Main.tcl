...
set inpath ../Ground_Motions; # changed here 
set indirlist {FEMA_P695_Far_Field}; # changed here
set sat1filename SaT1_2.5_4.78s.out;
set gminfofilename GMInfo.txt;

# Define the output path
set outpath /scratch/users/wyen/MRF31_191103; # changed here

# Define the collapse peak story drift ratio
set num_stories 35; # changed here
set num_bays 5; # changed here
set g 386.1; # changed here
set elemConvTolMax 1e-6; # changed here, tolerance
set col_drift 0.10;
...
if {$this_procid == 0} {
    ...
    # for scaling)
    # set periods_file [open Model_Info/periods.out r]; # changed here
    ...
    foreach indir $indirlist {
        ...
        set gminfofile [open $inpath/$indir/$gminfofilename r]
        # set sat1file [open $inpath/$indir/SaT1/SaT1_5_[format "%.3f" $T1]s.out r]; # changed here
        set sat1file [open $inpath/$indir/SaT1/$sat1filename r];
...
# Tasks to be executed by all slave processors
    ...
    # Source the required files, changed here
    # source constants_units_kip_in.tcl
    # source frame_data.tcl
    # source create_steel_mf_model.tcl
    source PanelZone2D2.tcl;
    source preNRBeam.tcl;
    source Wsection.tcl;
    source max_drift_check.tcl;

    # Define the list of nodes used to compute story drifts, changed here
    ...
    for {set i 0} {$status != "DONE"} {incr i} {
        ...
        # Wipe previous model definitions, create the model, and run the analysis, changed here
        # wipe;
        # CreateSteelMFModel frame_data.tcl
        # model BasicBuilder -ndm 2 -ndf 3;
        dict with gminfo {
            # source recorders_analysis_ida_mp_hpc.tcl; # changed here, run analysis
            # adjust element tolerance value until converge or reached the "elemConvTolMax", changed here
            set elemConvTol 1e-12;
            set state "inconvergence";
            while {$state == "inconvergence" && $elemConvTol <= $elemConvTolMax} {
                wipe; # changed here, moved wipe into the loop
                model BasicBuilder -ndm 2 -ndf 3;
                source MRF31.tcl;
                source DynamicAnalysis.tcl;
                set elemConvTol [expr $elemConvTol*100];
            }
        }
