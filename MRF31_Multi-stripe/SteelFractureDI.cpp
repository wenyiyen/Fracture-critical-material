		...
		if (eps >= epsCont) { // not contacted
			...
			*/
			...
			if (deps < 0) {
				...
				double epss0T = (Fy * shft - Esh * epsy * shft - sigrT + E0 * epsrT) / (E0 - Esh);
				double sigs0T = Fy * shft + Esh * (epss0T - epsy * shft);
				epspl = epsmax;
				// compute sig and e
				double xi = fabs((epspl - epss0T) / epsy);
				double R = R0 * (1.0 - (0.9*xi) / (0.15 + xi));
				double epsrat = (eps - epsrT) / (epss0T - epsrT);
				double dum1 = 1.0 + pow(fabs(epsrat), R);;
				double dum2 = pow(dum1, (1 / R));

				eps
				sig = b * epsrat + (1.0 - b)*epsrat / dum2;
				sig = sig * (sigs0T - sigrT) + sigrT;
