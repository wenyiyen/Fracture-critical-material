...
extern void *OPS_SteelFractureDI(void);
...
*/
	...
	if (argc < 4) {
		...
		opserr << " <-esu esu> <-k1 k1> <-k2 k2> <-db db> <-b1 b1> <-b2 b2>" << endln;
		return TCL_ERROR;
	}
	...
	for (int j = 4; j < argc; j++) {
		if (strcmp(argv[j], "-Dmax") == 0) {
		...
		else if (strcmp(argv[j], "-c_mono") == 0) {
			if ((j + 1 >= argc) ||
				(Tcl_GetDouble(interp, argv[j + 1], &c_mono) != TCL_OK)) {
				opserr << "WARNING invalid -c_mono";
				opserr << "uniaxialMaterial DuctileFracture: " << tag << endln;
				return TCL_ERROR;
			}
		}
		else if (strcmp(argv[j], "-c_cycl") == 0) {
			if ((j + 1 >= argc) ||
		...
		else if (strcmp(argv[j], "-c_symm") == 0) {
			if ((j + 1 >= argc) ||
			...
			if ((j + 1 >= argc) ||
				(Tcl_GetDouble(interp, argv[j + 1], &db) != TCL_OK)) {
				opserr << "WARNING invalid -db";
				opserr << "uniaxialMaterial DuctileFracture: " << tag << endln;
				return TCL_ERROR;
			}
		}
		else if (strcmp(argv[j], "-b1") == 0) {
			if ((j + 1 >= argc) ||
	...
	// Parsing was successful, allocate the material
	theMaterial = new DuctileFracture(tag, *theMat, Dmax, c_mono,
		c_cycl, c_symm, E_s, epsmin, epsmax, esu, k1, k2, db, b1, b2);
