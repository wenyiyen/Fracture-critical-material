...
SteelFractureDI::setTrialStrain(double trialStrain, double strainRate)
{
	...
	// variables in counting calculations
	std::copy(rev_cntP, rev_cntP+20, rev_cnt);
...
void
SteelFractureDI::calcDI(double sigcr, double m, int &isStart, int &isSecond, double sig, double &sigPDI, double &DI_cache, double &DI, double &slopeP, double *rev_cnt, int &rev_cnt_endIdx, int &isReadRev)
{
	...
	else if (sgn(slope) != sgn(slopeP) && sgn(slopeP) != 0) {
		rev_tmp[0] = sigPDI;
		rev_tmp[1] = sig_tmp;
		rev_tmp_endIdx = 1;
	}
	...
	while (rev_cnt_endIdx != -1) {
		if (isReadRev) {
			if (rev_tmp_endIdx == -1) {
				...
				// cache
				std::copy(rev_cnt, rev_cnt + 20, rev_cnt_cache);
				rev_cnt_endIdx_cache = rev_cnt_endIdx;
				isReadRev_cache = isReadRev;
				DI_cache = DI;
				// read
				rev_cnt_endIdx = rev_cnt_endIdx + 1;
				if (rev_cnt_endIdx > 20-1) {
					opserr << "Array 'rev_cnt' overflow, check material source code.\n";
				}
	...
	sigPDI = sig_tmp;
	slopeP = slope;
	std::copy(rev_cnt_cache, rev_cnt_cache + 20, rev_cnt);
...
SteelFractureDI::commitState(void)
{
	...
	slopePP = slopeP; // slope of previous stress history point
					  // variables in counting calculations
	std::copy(rev_cnt, rev_cnt+20, rev_cntP);
...
SteelFractureDI::sendSelf(int commitTag, Channel &theChannel)
{
	...
	data(31) = rev_cnt_endIdxP;
	data(32) = isReadRevP;
	for (int i = 0; i < 20; i++) {
		data(33 + i) = rev_cntP[i];
	}
	...
	FEM_ObjectBroker &theBroker)
{
	...
	DIP = data(25);
	DI_cacheP = data(26);
	isStartP = int(data(27));
	isSecondP = int(data(28));
	sigPDIP = data(29);
	slopePP = data(30); // slope of previous stress history point
	rev_cnt_endIdxP = int(data(31));
	isReadRevP = int(data(32));
	for (int i = 0; i < 20; i++) {
		rev_cntP[i] = data(33 + i); // NEED CHECK HERE!!!!!!
	}
